...
public class KafkaConnectionPoolTest {
    ...
    public void test() throws Exception {
        ...
        try {
            KafkaConnectionPool pool = new KafkaConnectionPool(new ConnectionPoolConfig(), prop);
            pool.close();
        } catch (Exception e) {
        }

        try {
            KafkaConnectionPool pool = new KafkaConnectionPool(new ConnectionPoolConfig(), config);
            pool.close();
        } catch (Exception e) {
        }

        try {
            KafkaConnectionPool pool = new KafkaConnectionPool(new ConnectionPoolConfig(), KafkaConfig.DEFAULT_BROKERS, KafkaConfig.DEFAULT_TYPE);
            pool.close();
        } catch (Exception e) {
        }
